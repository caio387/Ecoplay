...
// badges definitions
const BADGES = {
  econ: {id:'econ',icon:'💰',name_pt:'Economista Júnior',name_en:'Economist Junior',msg_pt:'Parabéns, {name}! Você virou um Economista Júnior 💰!', msg_en:'Congrats, {name}! You became an Economist Junior 💰!'},
  invest: {id:'invest',icon:'🏦',name_pt:'Investidor Esperto',name_en:'Smart Investor',msg_pt:'Parabéns, {name}! Você virou um Investidor Esperto 🏦!', msg_en:'Congrats, {name}! You became a Smart Investor 🏦!'},
  planner: {id:'planner',icon:'🧠',name_pt:'Planejador Mestre',name_en:'Master Planner',msg_pt:'Parabéns, {name}! Você conquistou Planejador Mestre 🧠!', msg_en:'Congrats, {name}! You got Master Planner 🧠!'},
  star: {id:'star',icon:'🌟',name_pt:'Super Aluno',name_en:'Super Student',msg_pt:'Parabéns, {name}! Você é um Super Aluno 🌟!', msg_en:'Congrats, {name}! You are a Super Student 🌟!'}
};
...
function playSaveSpend(){
  ...
  let score=0; const scenarios = [
    {desc:'Você recebeu R$20. O que faz?','options':[ {t:'Guardar R$15',p:10},{t:'Gastar com besteira',p:0} ]},
    {desc:'Precisa de material escolar. O que faz?','options':[ {t:'Comparar preços e poupar',p:10},{t:'Comprar a primeira opção',p:0} ]},
    {desc:'Quer um videogame caro. O que faz?','options':[ {t:'Criar plano de poupança',p:10},{t:'Parcelar sem pensar',p:0} ]},
  ]; let i=0; render();
...
function renderBadges(){
  ...
  Object.keys(BADGES).forEach(k=>{
    ...
    const el = document.createElement('div'); el.className='badge'; el.textContent = `${b.icon} ${state.lang==='pt'?b.name_pt:b.name_en}` + (unlocked? ' ✓':'');
...
async function renderTeacherReport(data){
  ...
  let html = '<table><thead><tr><th>Aluno</th><th>Turma</th><th>Jogo</th><th>Pontuação</th><th>Data</th><th>Badges</th></tr></thead><tbody>';
  ...
  html += `<div style="margin-top:12px"><strong>Média por turma:</strong> ${parts.join(' | ')}</div>`;
...
document.getElementById('refreshBtn').addEventListener('click', async ()=>{
  if(!window.profAuth) return alert('Professor não autenticado');
  const res = await API.getSessions(window.profAuth);
  const data = await res.json();
  renderTeacherReport(data);
});
document.getElementById('exportBtn').addEventListener('click', async ()=>{
  if(!window.profAuth) return alert('Professor não authenticated');
  const res = await API.getSessions(window.profAuth);
  const data = await res.json();
  const rows = [['Aluno','Turma','Jogo','Pontuação','Data']];
